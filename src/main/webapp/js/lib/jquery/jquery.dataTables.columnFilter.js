...
(function ($) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        return this.each(function () {
          ...
          var scrollable_parent = $(this).offsetParent(".dataTables_scroll");

          var context = oTable;
          var sFilterRow = "tfoot tr th";

            if (scrollable_parent.length > 0) {
               context = scrollable_parent.find(".dataTables_scrollHead table")[0]
               $(context).append("<tbody><tr/></tbody>");
               $("tbody tr:first td", oTable).each(function (index) {
            	   $(context).find("tbody tr").append("<td />");
               });
               sFilterRow = "tbody tr td";
            } else if (properties.sPlaceHolder == "head:after") {
                sFilterRow = "thead tr:last th";
            } else if (properties.sPlaceHolder == "head:before") {
                var tr = $("thead tr:last").detach();
                tr.prependTo("thead");
                sFilterRow = "thead tr:first th";
            }

            $(sFilterRow , context).each(function (index) {
                i = index;
